...
public class NPCCarLv2 : MonoBehaviour
{
    ...
    [Header("Pathfinding")]
    public TrafficNodeLv2 currentNode;
    ...
    {
        NPCStopLineLv2 stopLine = other.GetComponent<NPCStopLineLv2>();
        if (stopLine != null)
        {
            if (stopLine.stopType == NPCStopLineLv2.StopType.TrafficLight)
                StartCoroutine(TrafficLightRoutine(stopLine));
            else if (stopLine.stopType == NPCStopLineLv2.StopType.StopSign)
                StartCoroutine(StopSignRoutine(stopLine));
        }
    }

    IEnumerator TrafficLightRoutine(NPCStopLineLv2 stopLine)
    {
    ...
    IEnumerator StopSignRoutine(NPCStopLineLv2 stopLine)
    {
        isStoppedByTraffic = true;
